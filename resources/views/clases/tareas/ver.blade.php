...
@section('body')
    ...
    <div style="display: flex; margin: 2rem; justify-content: space-around;">
        <div class="sombra2" style="border-radius: 5px; padding: 1rem; width: 73%;">
            <div
                style="margin-left: 10px; margin-right: 10px; margin-top: 5px; display: flex; justify-content: space-between;">
                <div>
        ...
        @can('clases.tareas.alumno.upload')
            @if ($homework->pivot->status == 'No Entregada')
                <div class="" style="width: 25rem;">
                    <form action="{{ route('clases.tareas.subir', [$lesson, $homework]) }}" method="POST"
                        enctype="multipart/form-data" class="sombra1"
                        style="border-radius: 5px;  margin-bottom: 1rem; height: 15rem;">
                        @csrf
                        @method('put')
                        <div style="padding: 1rem;">
                            <div style=" display: flex; align-items: center; margin-left: 10px; margin-top: 1rem;">
                                ...
                                <input
                                    ...
                                    id="files" type="file" multiple name="files[]">
                                <label for="files"
                                    class="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded text-white focus:outline-none">Seleccionar</label>
                                </input>
                                @error('files.*')
                                    {{ $message }}
                                @enderror
                            ...
                            <div style="margin-left: 10px; margin-top: 3rem;">
                                <button
                                    class="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded text-white focus:outline-none">Entregar
                                    tarea</button>
                            </div>
                        </div>
                    </form>
            ...
            @else
                ...
                <div class="" style="width: 25rem;">
                    <form action="{{ route('clases.tareas.cancel', [$lesson, $homework]) }}" method="POST"
                        enctype="multipart/form-data" class="sombra1"
                        style="border-radius: 5px;  margin-bottom: 1rem; height: 15rem;">
                        @csrf
                        @method('put')
                        <div style="padding: 1rem;">
                            <div style="margin-left: 10px; margin-top: 3rem;">
                                <button
                                    class="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded text-white focus:outline-none">Anular
                                    entrega
                                </button>
                            </div>
                        </div>
