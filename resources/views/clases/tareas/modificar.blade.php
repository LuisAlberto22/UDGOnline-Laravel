        ...
        <form method="POST" enctype="multipart/form-data" action="{{ route('clases.tareas.update', [$lesson,$homework]) }}">
            ...
            <div style="display: colum; align-items: center;">
                @foreach ($users as $user)
                     <input type ="checkbox" @if ($homework->users()->find($user->id) != null)
                         checked
                     @endif  name="users[]" value="{{$user->id}}">{{$user->name}}</input>
                @endforeach
                @error('users')
                    {{$message}}
                @enderror
            ...
            <div style="direction: rtl;">
                <button class="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded text-white focus:outline-none">Modificar</button>
            </div>
        </form>
