...
<body>
    ...
    <form method="POST" enctype="multipart/form-data" action="{{route('clases.tareas.update' , [$lesson,$homework])}}">
        ...
        <div class="mb-4">
            ...
            <input type="text" class="border-2 border-gray-300 p-2 w-full" name="name" id="title" value="{{$homework->name}}" required></input>
        </div>
        @error("name")
            {{ $message }}
        @enderror
    ...
    @enderror
        <div style="margin-top: .5rem;">
            ...
            <input class="bg-gray-300 " disabled width="3rem" value="{{date("Y-m-d\TH:i:s", strtotime($homework->delivery_date))}}" type="datetime-local" id="start" name="delivery_date" >
        </div>
